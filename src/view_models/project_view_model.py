...
class ProjectViewModel:
    ...
    def __init__(self, user_project_folder):
        # ToDo - add trait name automatically to file output path
    ...
    def run(self, id_term, name_term, include_child):
        ...
        self._pipeline = self._run_pipeline(associations_config, ld_config, logger, id_term, name_term, include_child)
        return 0

    def _run_pipeline(self, associations_config, ld_config, logger, id_term, name_term, include_child):
        start = time.time()
        ...
        pipeline.run(id_term, name_term, include_child)
